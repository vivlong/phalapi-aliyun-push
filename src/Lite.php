    ...
    private function rpcRequest($action, $params)
    {
        ...
        try {
            ...
            if ($result->isSuccess()) {
                ...
                $di->logger->info(__NAMESPACE__, __FUNCTION__, ['Response' => $response['body']]);
            } else {
                $rs['code'] = -1;
                $rs['msg'] = $result;
                $di->logger->info(__NAMESPACE__, __FUNCTION__, ['Failed' => $result]);
            }
        } catch (ClientException $e) {
            $di->logger->error(__NAMESPACE__, __FUNCTION__, ['ClientException' => $e->getErrorMessage()]);
            $rs['code'] = -1;
            $rs['msg'] = $result;
            $di->logger->error(__NAMESPACE__, __FUNCTION__, ['Failed' => $result]);
        } catch (ServerException $e) {
            $di->logger->error(__NAMESPACE__, __FUNCTION__, ['ServerException' => $e->getErrorMessage()]);
            $rs['code'] = -1;
            $rs['msg'] = $result;
            $di->logger->error(__NAMESPACE__, __FUNCTION__, ['Failed' => $result]);
        }
